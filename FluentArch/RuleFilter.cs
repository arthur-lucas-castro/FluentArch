...
namespace FluentArch
{
    public class RuleFilter : IFilters
    {
        ...
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => classe.Nome.StartsWith(startingName));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder); ;
        }
        public ILayer HaveNameStartingWith(string startingName, StringComparison stringComparison)
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => classe.Nome.StartsWith(startingName, stringComparison));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder);
        }
        ...
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => classe.Nome.EndsWith(endingName));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder);
        }
        public ILayer HaveNameEndingWith(string endingName, StringComparison stringComparison)
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => classe.Nome.EndsWith(endingName, stringComparison));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder);
        }
        public ILayer HaveNameMatchingWith(string pattern, RegexOptions regexOptions)
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => Regex.IsMatch(classe.Nome, pattern, regexOptions));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder);
        }
        public ILayer HaveNameMatchingWith(string pattern)
        {
            var classesFiltradas = _builder.GetTypes().Where(classe => Regex.IsMatch(classe.Nome, pattern));
            _builder.UpdateTypes(classesFiltradas.ToList());
            return new Layer(_builder.GetTypes(), _builder);
        }
